...
export default function CaseStudyModal({ project, onClose }: CaseStudyModalProps) {
  ...
  const getNodeIcon = (type: string) => {
    switch (type) {
      case "user": return <Activity className="w-4.5 h-4.5 text-[#5de6ff]" />;
      case "balancer": return <RefreshCcw className="w-4.5 h-4.5 text-[#ffb869]" />;
      case "gateway": return <Network className="w-4.5 h-4.5 text-[#d0bcff]" />;
      case "cache": return <Layers className="w-4.5 h-4.5 text-[#5de6ff]" />;
      case "database": return <Server className="w-4.5 h-4.5 text-[#d0bcff]" />;
  ...
  return (
    ...
    >
      <div 
        className="glass-card inner-glow w-full max-w-5xl rounded-2xl relative shadow-[0_0_50px_rgba(109,59,215,0.25)] flex flex-col max-h-[90vh] overflow-hidden animate-zoom-in"
        id="case-study-modal-viewport"
      >
        {/* Header Ribbon */}
        <div className="flex justify-between items-center border-b border-white/10 px-6 py-4 bg-[#15121b]/80">
          <div className="flex items-center gap-3">
            <span className="w-1.5 h-6 bg-[#d0bcff] rounded-full" />
          ...
          <button 
            onClick={onClose}
            className="p-1 rounded-md text-[#cbc3d7] hover:text-white hover:bg-white/5 border border-white/5 transition-colors"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="aspect-video w-full rounded-xl overflow-hidden relative border border-white/10">
                ...
                <div className="absolute bottom-4 left-4 flex gap-2">
                  {project.tags.slice(0, 4).map(t => (
                    <span key={t} className="px-2.5 py-0.5 rounded bg-black/80 border border-white/10 text-[9px] font-mono tracking-wide text-[#5de6ff]">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-4">
                <h5 className="font-mono text-xs tracking-widest text-[#d0bcff] uppercase">SYSTEM_OVERVIEW</h5>
                <h3 className="font-sans font-extrabold text-[#5de6ff] text-2xl leading-tight">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                {project.metrics.map((metric, idx) => (
                  <div key={idx} className="bg-white/[0.02] border border-white/5 p-3 rounded-lg flex flex-col justify-between">
                    <span className="font-mono text-[9px] text-[#958ea0] tracking-wider uppercase leading-none mb-2">
                      {metric.label}
                    </span>
                    <div className="flex items-baseline gap-1.5">
                      ...
                      {metric.trend === "up" && (
                        <span className="text-[10px] text-[#5de6ff] font-sans">▲</span>
                      )}
                      {metric.trend === "down" && (
                        <span className="text-[10px] text-[#ffb869] font-sans">▼</span>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="bg-[#1d1a23]/60 border border-white/5 p-5 rounded-xl flex items-center justify-between gap-4">
                <div>
                  <h5 className="font-mono text-[10px] text-[#d0bcff] tracking-widest uppercase mb-1">BENCHMARK_SCORE</h5>
                  <h4 className="font-sans font-extrabold text-xl text-white">OPTIMIZATION_RATING</h4>
                  <p className="text-[10px] text-[#cbc3d7] mt-1">Calculated via memory throughput allocation protocols.</p>
                </div>
                <div className="relative flex items-center justify-center w-20 h-20 flex-shrink-0">
                  <svg className="w-full h-full -rotate-90">
                    ...
                    <circle 
                      ...
                      cy="40" 
                      r="34" 
                      className="stroke-[#5de6ff] fill-none transition-all duration-1000" 
                      strokeWidth="4" 
                      strokeDasharray="213" 
              ...
              <div className="glass-card inner-glow p-5 rounded-xl space-y-4">
                <div className="flex justify-between items-center border-b border-white/5 pb-3">
                  <h6 className="font-mono text-[10px] text-[#5de6ff] tracking-widest uppercase flex items-center gap-2">
                    <Activity className="w-3.5 h-3.5 animate-pulse text-[#5de6ff]" /> LIVE_DIAGNOSTICS
                  </h6>
                  <span className="font-mono text-[9px] text-[#958ea0] tracking-wider uppercase">V-HOST RUNNING</span>
                </div>
                ...
                <div className="space-y-3 font-mono text-xs">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px]">
                      <span className="text-[#cbc3d7]">PROCESSOR THREAD</span>
                      <span className="text-white font-bold">{cpuLoad}%</span>
                    </div>
                    <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                      <div 
                        className="bg-gradient-to-r from-[#d0bcff] to-[#5de6ff] h-full transition-all duration-300"
                        style={{ width: `${cpuLoad}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between border-b border-white/5 py-1 text-[10px]">
                    <span className="text-[#cbc3d7]">CLUSTER TELEMETRY JITTER</span>
                  ...
                  <div className="flex justify-between py-1 text-[10px]">
                    <span className="text-[#cbc3d7]">STABILITY MARGIN (REFRESH RATE)</span>
                    <span className="text-[#5de6ff] font-bold">{fpsVal}fps</span>
                  </div>
                </div>
              </div>
              ...
              <div className="glass-card inner-glow p-5 rounded-xl space-y-4" id="topology-visualizer">
                <div className="flex justify-between items-center">
                  <h6 className="font-mono text-[10px] text-white tracking-widest uppercase flex items-center gap-2">
                    <Zap className="w-3.5 h-3.5 text-[#ffb869]" /> INTERACTIVE_TOPOLOGY_GRAPH
                  </h6>
                  <span className="text-[9px] font-mono text-[#958ea0]">SELECT NODE TO INSPECT</span>
                </div>
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          ...
                          <div className="font-mono text-[9px] text-[#958ea0] uppercase leading-none">{node.type}</div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#5de6ff] animate-pulse" />
                        <span className="font-mono text-[8px] text-[#958ea0]">ACTIVE</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {selectedNode && (
                  <div className="bg-[#15121b]/80 border border-white/5 p-3 rounded-lg space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="font-mono text-[9px] text-[#5de6ff] uppercase tracking-wider font-bold">NODE_SPEC_DATA</span>
                      <span className="font-mono text-[8px] text-white/55">HEALTH: 100%</span>
                    </div>
                    <div className="font-mono text-[11px] space-y-1">
                      <div><span className="text-[#958ea0]">MODULE:</span> <span className="text-white">{selectedNode.name}</span></div>
                      <div><span className="text-[#958ea0]">OBJECT ID:</span> <span className="text-[#ffb869] text-[10px]">{selectedNode.id}</span></div>
                      <div><span className="text-[#958ea0]">ALLOCATION:</span> <span className="text-white uppercase text-[10px]">{selectedNode.type} layer</span></div>
                    </div>
                  </div>
                )}
          ...
          <div className="glass-card inner-glow p-5 rounded-xl space-y-3" id="diagnostics-terminal">
            <div className="flex justify-between items-center border-b border-white/5 pb-3">
              ...
              <button 
                ...
                className="flex items-center gap-1 px-3 py-1 font-mono text-[9px] text-[#5de6ff] bg-[#5de6ff]/5 border border-[#5de6ff]/20 rounded hover:bg-[#5de6ff]/10 disabled:opacity-40 transition-colors uppercase cursor-pointer"
              >
                <RefreshCcw className={`w-3 h-3 ${isRebooting ? "animate-spin" : ""}`} /> Reboot Pipeline
              </button>
            </div>

            <div className="bg-[#0f0d15] p-4 rounded border border-white/5 h-48 overflow-y-auto font-mono text-[11px] leading-relaxed text-[#5de6ff]/90 space-y-1">
              {liveLogs.map((log, lIdx) => (
                <div key={lIdx} className="flex gap-2.5 items-start">
                  <span className="text-[#958ea0] select-none">[{lIdx + 1}]</span>
                  <p className="text-left text-[#5de6ff]">{log}</p>
                </div>
              ))}
              <div ref={logsEndRef} />
            </div>
        ...
        {/* Footer Ribbon */}
        <div className="flex justify-end gap-3 border-t border-white/10 px-6 py-4 bg-[#15121b]/80">
          <button 
            onClick={onClose}
            className="px-4 py-2 rounded border border-white/10 text-[#cbc3d7] font-mono text-xs hover:border-white/20 hover:text-white transition-colors uppercase cursor-pointer"
