  ...
  return (
    <div className="space-y-12" id="network-hub-container">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-5xl mx-auto">
        <div className="bg-[#1d1a23]/60 border border-white/5 p-4 rounded-xl">
          <span className="font-mono text-[9px] tracking-wider text-[#958ea0] uppercase">AVERAGE_LATENCY</span>
          <h3 className="font-mono text-xl md:text-2xl text-[#5de6ff] font-bold mt-1">{networkLatency} ms</h3>
          <p className="text-[10px] text-[#cbc3d7] mt-1 flex items-center gap-1">
            <ShieldCheck className="w-3" /> Optimum stability index
          </p>
        </div>

        <div className="bg-[#1d1a23]/60 border border-white/5 p-4 rounded-xl">
          <span className="font-mono text-[9px] tracking-wider text-[#958ea0] uppercase">ACTIVE_REGIONS</span>
          <h3 className="font-mono text-xl md:text-2xl text-white font-bold mt-1">5 / 6 ONLINE</h3>
          <p className="text-[10px] text-[#cbc3d7] mt-1 flex items-center gap-1 text-[#ffb869]">
            <AlertTriangle className="w-3 text-[#ffb869]" /> Cape Town offline
          </p>
        </div>

        <div className="bg-[#1d1a23]/60 border border-white/5 p-4 rounded-xl">
          <span className="font-mono text-[9px] tracking-wider text-[#958ea0] uppercase">GLOBAL_FIREWALL</span>
          <h3 className="font-mono text-xl md:text-2xl text-[#d0bcff] font-bold mt-1">ENCRYPTED</h3>
          <p className="text-[10px] text-white/55 mt-1">Zero Trust TLS 1.3 tunnels</p>
        </div>

        <div className="bg-[#1d1a23]/60 border border-white/5 p-4 rounded-xl">
          <span className="font-mono text-[9px] tracking-wider text-[#958ea0] uppercase">PACKET_LOSS_RATING</span>
          <h3 className="font-mono text-xl md:text-2xl text-[#5de6ff] font-bold mt-1">0.02%</h3>
          <p className="text-[10px] text-[#cbc3d7] mt-1">Optimized traffic structures</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 max-w-5xl mx-auto">
        ...
        <div className="lg:col-span-6 space-y-4">
          <h4 className="font-mono text-xs tracking-widest text-[#d0bcff] uppercase pl-1">VIRTUAL_SERVER_HOSTS</h4>
              ...
              <div
                ...
                className={`glass-card inner-glow p-4 rounded-xl border text-left cursor-pointer transition-all duration-300 flex items-center justify-between ${
                  selectedHost?.id === host.id
                    ? "bg-[#5de6ff]/5 border-[#5de6ff]/60 shadow-[0_0_20px_rgba(93,230,255,0.08)]"
                    : "bg-white/[0.01] border-white/5 hover:bg-white/[0.03]"
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className="bg-white/5 p-2 rounded-lg">
                    <Server className={`w-4 h-4 ${host.status === 'online' ? 'text-[#5de6ff]' : host.status === 'degraded' ? 'text-[#ffb869]' : 'text-gray-600'}`} />
                  </div>
                  <div>
                    <h5 className="font-mono font-bold text-xs text-white leading-none mb-1.5">{host.name}</h5>
                    <span className="font-mono text-[9px] text-[#958ea0] bg-white/5 px-1.5 py-0.5 rounded mr-1">
                      {host.region}
                    </span>
                ...
                <div className="text-right">
                  <span className={`inline-flex items-center gap-1 text-[10px] font-mono uppercase tracking-wider ${
                    host.status === 'online' ? 'text-[#5de6ff]' : host.status === 'degraded' ? 'text-[#ffb869]' : 'text-[#ffb4ab]'
                  }`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      host.status === 'online' ? 'bg-[#5de6ff] animate-pulse' : host.status === 'degraded' ? 'bg-[#ffb869]' : 'bg-[#ffb4ab]'
                    }`} />
                    {host.status}
                  </span>
                  {host.status !== "offline" && (
        ...
        <div className="lg:col-span-6 space-y-4">
          <h4 className="font-mono text-xs tracking-widest text-[#d0bcff] uppercase pl-1">DIAGNOSTICS_VIEW_PORT</h4>
          ...
          {selectedHost ? (
            <div className="glass-card inner-glow p-6 rounded-2xl border border-white/5 space-y-6 flex flex-col h-full justify-between" id="selected-server-card">
              <div className="space-y-4">
                <div className="flex justify-between items-start border-b border-white/5 pb-4">
                  <div>
                    <span className="font-mono text-[9px] text-[#5de6ff] uppercase tracking-widest font-bold">NODE ID: {selectedHost.id}</span>
                    <h3 className="font-sans font-extrabold text-[#d0bcff] text-xl md:text-2xl mt-1">{selectedHost.name}</h3>
                  </div>
                  <button
                    onClick={() => handleTraceroute(selectedHost)}
                    disabled={isTracing || selectedHost.status === "offline"}
                    className="flex items-center gap-1.5 px-3 py-1.5 font-mono text-[9px] text-[#5de6ff] bg-[#5de6ff]/5 border border-[#5de6ff]/25 rounded hover:bg-[#5de6ff]/10 disabled:opacity-40 transition-colors uppercase cursor-pointer"
                  >
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1 bg-[#15121b]/80 border border-white/5 p-3 rounded-lg">
                    <div className="flex justify-between text-[10px] font-mono text-[#958ea0] mb-1">
                      <span>AVERAGE CPU LOAD</span>
                      <span className="text-white font-bold">{selectedHost.cpuLoad}%</span>
                    </div>
                    <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#5de6ff] h-full transition-all duration-300" style={{ width: `${selectedHost.cpuLoad}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1 bg-[#15121b]/80 border border-white/5 p-3 rounded-lg">
                    ...
                    <div className="w-full bg-white/5 h-1.5 rounded-full overflow-hidden">
                      <div className="bg-[#d0bcff] h-full transition-all duration-300" style={{ width: `${selectedHost.memoryLoad}%` }} />
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-[9px] text-[#cbc3d7] tracking-wider uppercase">DYNAMIC TRACEROUTE KERNEL</span>
                    <span className="font-mono text-[8px] text-[#958ea0]">STABLE CONNECTION</span>
                  </div>

                  <div className="bg-[#0f0d15] p-4 rounded-xl border border-white/5 h-48 overflow-y-auto font-mono text-[10.5px] leading-relaxed text-[#5de6ff] space-y-2">
                    {tracerouteLogs.map((log, lIdx) => (
                      <div key={lIdx} className="flex gap-1.5 items-start">
                        <CornerDownRight className="w-3 h-3 text-[#cbc3d7]/60 mt-0.5 flex-shrink-0" />
              ...
              <div className="text-[10px] font-mono text-[#cbc3d7] text-left pt-2 border-t border-white/5 mt-4">
            ...
            <div className="glass-card text-center py-16 rounded-2xl border border-white/10 flex items-center justify-center">
              <span className="text-[#cbc3d7] font-mono text-xs">No host module targeted. Choose server node.</span>
            </div>
          )}
        </div>
      </div>
